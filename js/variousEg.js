...
$(function () {
  ...
  function ReadyMarket(data) {
    ...
    $(_firstMarket).each(function (i, items) {
      ...
      if (items.status === 1) {
        readyH2.eq(2).text('Ongoing maintenance');
      } else if (items.status === 0) {
        readyH2.eq(2).text('Pause maintenance');
      } else {
        return false;
      }

      if (items.lastStatus === 1) {
        readyH2.eq(3).text('The last period has been maintained');
      }	else if (items.lastStatus === 0) {
        readyH2.eq(3).text('Not yet maintained');
      }	else {
        return false;
      }
    });
  ...
  function GetImg(items, i, whichMarket) {
    ...
    if (items.marketName === 'Local market') {
      getImg.attr('src', '../../images/native_icon.gif');
    } else if (items.marketName === 'Regional market') {
      getImg.attr('src', '../../images/region_icon.gif');
    }	else if (items.marketName === 'Domestic market') {
      getImg.attr('src', '../../images/china_icon.gif');
    }	else if (items.marketName === 'international market') {
      getImg.attr('src', '../../images/internation_icon.gif');
    } else if (items.marketName === 'Asia market') {
  ...
  function Update(Open, items) {
    ...
    if (items.status === 1) {
      openHh2.eq(5).text('Is developing');
      Open.find('h2:last').text('Suspension of development');
    }	else if (items.status === 0) {
      openHh2.eq(5).text('Suspension of development');
      Open.find('h2:last').text('Continue to develop');
    } else {
      return false;
    }
  }
  ...
  $(document).on('click', '.readyOpenBtn', function () {
    ...
    if (_h2.text() === 'Pause maintenance') {
      _h2.text('Start maintenance');
      prevH2.eq(2).text('Pause maintenance');
      Submit('marketAction!stopDevelopedMarket.action', prevH2);
    } else {
      _h2.text('Pause maintenance');
      prevH2.eq(2).text('Ongoing maintenance');
  ...
  $(document).on('click', '.openBtn', function () {
    ...
    if (_hh2.text() === 'Suspension of development') {
      _hh2.text('Continue to develop');
      prevHh2.eq(5).text('Suspension of development');
      Submit('marketAction!stopDevelopingMarket.action', prevHh2);
    } else {
      _hh2.text('Suspension of development');
      prevHh2.eq(5).text('Continue to develop');
