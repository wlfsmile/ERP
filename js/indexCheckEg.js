  ...
  loginCheck: function () {
    ...
    var userCheck = function userCheck(_this1) {
      if ($(_this1).val().search(/\s+/) === 0 || $(_this1).val() === '') {
        $('.errorInfo').empty().append('username can not be empty').css('color', 'orange');
        ok1 = false;
      } else {
        ok1 = true;
      }
    ...
    var pswCheck = function pswCheck(_this2) {
      if ($(_this2).val().search(/\s+/) === 0 || $(_this2).val() === '') {
        $('.errorInfo').empty().append('password can not be blank').css('color', 'orange');
        ok2 = false;
      } else {
        $('.errorInfo').empty();
        ok2 = true;
  ...
  createGroup: function () {
    ...
    var groupname = function groupname(_this) {
      ...
      if ($(_this).val().search(/\s+/) === 0 || $(_this).val() === '') {
        ...
        $('.error').empty().append(str + 'Can not be empty').css('color', 'orange');
        ok1 = false;
      } else {
        // 不为空时，取得输入组名和已存在组名判断
        $.get('gameGroupAction!isGameGroupExist.action?rnd=' + Math.random(), {
          ...
          if (data === true) {
            ...
            $('.error').empty().append('The group name already exists');
            ok2 = false;
            return false;
          }
        }, 'json');
    ...
    var creategroupCheck = function creategroupCheck(_this) {
      ...
      if ($(_this).val().search(/\s+/) === 0 || $(_this).val() === '') {
        ...
        $('.error').empty().append('Please enter the correct format').css('color', 'orange');
        ok1 = false;
      } else {
        $(_this).next()
          .empty()
