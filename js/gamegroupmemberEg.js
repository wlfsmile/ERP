...
function displayGameGroupMemberList(list) {
	...
	//头部信息展示
    	$.post("gameGroupAction!getGameGroupMessage.action?rnd="+Math.random(),function(data){
			...
			$('.header_tab td').eq(0).text("group name："+data.groupName);
			$('.header_tab td').eq(1).text("founder："+data.groupCreaterId);
			$('.header_tab td').eq(2).text("number of years："+data.years);
			$('.header_tab td').eq(3).text("number of cycles："+data.periodsOfOneYear);
          },"json");
		        ...
		        $.each(obj,function(indx){
                 ...
                 $('.left_infor').append(sp2);
			        if($(_this).next().text()==obj[indx].name){
			            ...
			            $(targ2).find('p:eq(0)').text("username："+obj[indx].userID);
			            $(targ2).find('p:eq(1)').text("realName："+obj[indx].name);
			            $(targ2).find('p:eq(2)').text("major："+obj[indx].major);
			            $(targ2).find('p:eq(3)').text("class："+obj[indx].className);
			            $(targ2).find('p:eq(4)').text("tel："+obj[indx].tel);
			            $(targ2).find('p:eq(5)').text("email："+obj[indx].email);
			            return false;
			        }
			    });
		    });
...
$(function(){
	...
	$('input[name="remove_member"]').click(function(){
	   ...
	   if(str=="") alert("Please choose to delete inside");
	   else{
